...
container = docker_client.containers.get("geoserver_stack_geoserver.1.lu7ybiv7xiw38yl7itgow0mvg")
service = docker_client.services.get("geoserver_stack_geoserver")

# Repetir monitoramento e ativação.
count = 1
while (True):
	text = container.stats(stream=False)
	...
	# Calcular deltas.
	cpu_delta = int(array[102].split(",")[0].lstrip()) - int(array[114].split(",")[0].lstrip())
	system_cpu_delta = int(array[106].split(",")[0].lstrip()) - int(array[118].split(",")[0].lstrip())
	online_cpus = int(array[107].split(",")[0].lstrip())
	...
	service_replicas = int(str(api_client.inspect_service(service="geoserver_stack_geoserver")).split(":")[45].split(",")[0].lstrip()[0])
