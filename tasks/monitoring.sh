...
do
	# Collecting Metrics.
	sudo curl --unix-socket /var/run/docker.sock "http://localhost/v1.41/containers/{dockercomposefiles_web_1}/stats?stream=false&one-shot=true" > ./statistics/stats.txt
	cut -d ":" -f 71 ./statistics/stats.txt 1> ./statistics/precpu_total_usage.txt
	cut -d "," -f 1 ./statistics/precpu_total_usage.txt 1> ./statistics/precpu_total_usage2.txt

	cut -d ":" -f 75 ./statistics/stats.txt 1> ./statistics/presystem_cpu_usage.txt
	cut -d "," -f 1 ./statistics/presystem_cpu_usage.txt 1> ./statistics/presystem_cpu_usage2.txt

	cut -d ":" -f 76 ./statistics/stats.txt 1> ./statistics/preonline_cpus.txt
	cut -d "," -f 1 ./statistics/preonline_cpus.txt 1> ./statistics/preonline_cpus2.txt

	sudo curl --unix-socket /var/run/docker.sock "http://localhost/v1.41/containers/{dockercomposefiles_web_1}/stats?stream=false&one-shot=true" > ./statistics/stats2.txt
	cut -d ":" -f 71 ./statistics/stats2.txt 1> ./statistics/cpu_total_usage.txt
	cut -d "," -f 1 ./statistics/cpu_total_usage.txt 1> ./statistics/cpu_total_usage2.txt

	cut -d ":" -f 75 ./statistics/stats2.txt 1> ./statistics/system_cpu_usage.txt
	cut -d "," -f 1 ./statistics/system_cpu_usage.txt 1> ./statistics/system_cpu_usage2.txt

	cut -d ":" -f 76 ./statistics/stats2.txt 1> ./statistics/online_cpus.txt
	...
	then
		...
		then
			sudo docker-compose --file dockercomposefiles/docker-compose.yml scale web=3
		#else
		#	sudo docker-compose scale web=1
		fi
	fi
